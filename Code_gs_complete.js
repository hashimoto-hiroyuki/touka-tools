...
function addSpecimen(params) {
  ...
  if (sheet.getLastRow() < 1) {
    sheet.getRange(1, 1, 1, 10).setValues([['No.', '医療機関名', '名前', '生年月日', '歯の位置', 'PEN', 'PYD', 'PpP値', '分析日', '備考']]);
  }
...
function getKnowledgeText() {
  ...
  '- カスタムプレフィックス・連番設定\n\n' +
  '#### PDF結合・表裏統合（merge_pdf.html）\n' +
  '- 表面と裏面が別々にスキャンされたPDFを1つの2ページPDFに結合\n' +
  '- pdf-libでブラウザ上で結合（サーバー不要）\n' +
  '- 1組モード：表面・裏面を個別にドロップして結合\n' +
  '- バッチモード：複数PDFをファイル名順でペアリングし一括結合\n' +
  '- Google Driveアップロードまたはローカルダウンロード\n\n' +
  '#### PDFページ数チェック（check_pdf_pages.html）\n' +
  '- 元PDFフォルダ内の全PDFが2ページセット（表裏）になっているか一括チェック\n' +
  '- GAS getDrivePdfList + getPdfPageCountで各ファイルのページ数を取得\n' +
  '- 2ページ以外のファイルを異常として検出・警告表示\n' +
  '- サマリーカード（合計/正常/異常）、フィルタ機能、PDFビューア・結合ツールへのリンク\n\n' +
  '### カテゴリ2: OCR読み取り・照合\n\n' +
...
function indexPdfWithGemini(fileId) {
  ...
  var prompt = 'あなたは日本の医療アンケートのOCRアシスタントです。\n' +
    ...
    '5. QRコード回答チェック: 「質問2」の生年月日欄の近くにある「□以降、QRコードで回答」というチェックボックスの判定\n\n' +
    '【qrCheckbox判定の重要な基準】\n' +
    'この判定は非常に重要です。以下の基準に厳密に従ってください。\n' +
    '- チェック「なし」の状態: □の中が空白で何も書かれていない。□の輪郭線のみが見える。印刷された□がそのまま残っている。\n' +
    '- チェック「あり」の状態: □の中や上にペンで書かれたレ点（✓）、×印、斜め線、塗りつぶし、丸囲みなど、手書きの何らかの印がある。□の形が崩れるほど上から書き込まれている場合もチェックありとする。\n' +
    '- 判定のコツ: □の周囲をよく見て、印刷された線以外に手書きのインクの跡があるかどうかで判断する。チェックボックスは小さいので注意深く確認すること。\n' +
    '- 迷った場合はfalse（チェックなし）としてください。\n' +
    '- チェックボックス自体が存在しない（旧フォーマット）場合もfalseとしてください。\n\n' +
